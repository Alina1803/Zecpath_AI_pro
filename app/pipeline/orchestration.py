...
class HiringOrchestrator:
    ...
    def process_ats(
        ...
        crud.save_ats_score(
            db=self.db, candidate_id=candidate_id, job_id=job_id, scores=ats_result
        )
    ...
    def process_interview(self, candidate_id: str, transcript: str):
        ...
        crud.save_interview_score(
            db=self.db, candidate_id=candidate_id, scores=interview_result
        )
        ...
        crud.create_candidate(
            db=self.db, candidate_id=candidate_id, name=name, email=email
        )
        ...
        ats_result = self.process_ats(
            candidate_id, job_id, parsed_resume, job_requirements
        )

        # Interview
        interview_result = self.process_interview(candidate_id, interview_transcript)
