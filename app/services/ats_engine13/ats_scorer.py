...
from app.services.ats_engine13.domain_detector import (
    detect_ca_domain
)
...
def safe_score(value):
    ...
    if isinstance(value, dict):
        ...
        if "total_experience_months" in value:
            ...
            return round(
                value["total_experience_months"] / 12,
                1
            )
...
def calculate_skill_score(
    ...
    jd_score = (
        len(resume_set & jd_set)
        / len(jd_set)
        * 100
    ) if jd_set else 0

    domain_score = (
        len(resume_set & domain_set)
        / len(domain_set)
        * 100
    ) if domain_set else 0
...
def calculate_education_score(
    ...
    resume_set = set(
        normalize_list(resume_edu)
    )

    jd_set = set(
        normalize_list(jd_edu)
    )
...
def calculate_score(
    ...
    domain_analysis = detect_ca_domain(
        resume_data
    )
