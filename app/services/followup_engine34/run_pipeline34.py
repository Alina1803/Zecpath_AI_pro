...
def run():
    ...
    for i in range(TOTAL_QUESTIONS):
        ...
        try:
            score = scoring_engine.evaluate(question, answer) if scoring_engine else 5
        except Exception as e:
        ...
        if decision.should_followup(level):
            ...
            if followup_q:
                ...
                try:
                    followup_score = scoring_engine.evaluate(followup_q, followup_answer) if scoring_engine else 5
                except Exception as e:
                    print(f" Follow-up scoring failed: {e}")
                    followup_score = 5

                # Save follow-up response
                state.update(followup_q, followup_answer, followup_score, followup=True)
