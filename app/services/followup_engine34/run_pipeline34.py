...
def load_scoring_dependencies(role):
    ...
    if not os.path.exists(domain_path):
        ...
        raise FileNotFoundError(f"Missing domain.json at " f"{domain_path}")

    if not os.path.exists(prompt_path):

        raise FileNotFoundError(f"Missing prompt.txt at " f"{prompt_path}")
    ...
    def run(self, role, experience):
        ...
        for i in range(self.total_questions):
            ...
            try:
                ...
                score = (
                    scoring_engine.evaluate(question, answer) if scoring_engine else 5
                )
            ...
            if decision.should_followup(level):
                ...
                followup_q = followup_generator.generate(question, answer, level)

                if followup_q:

                    print(f"\nFollow-up: " f"{followup_q}")

                    followup_answer = input("Your Answer: ").strip()

                    try:

                        followup_score = (
                            scoring_engine.evaluate(followup_q, followup_answer)
                            if scoring_engine
                            else 5
                        )
                        ...
                        print(f"Follow-up " f"scoring failed: {e}")

                        followup_score = 5

                    state.update(
                        followup_q, followup_answer, followup_score, followup=True
                    )
...
def run():
    ...
    pipeline = LegacyFollowupPipeline()

    pipeline.run(role=role, experience=experience)
