...
def extract_skills(text: str) -> Dict[str, List[str]]:
    ...
    if not text or not isinstance(text, str):
        return {"technical_skills": [], "accounting_skills": [], "soft_skills": []}

    cleaned = clean_text(text)

    categorized = {"technical_skills": [], "accounting_skills": [], "soft_skills": []}
...
def match_skills(resume_skills: Dict, jd_skills: Dict) -> Dict:
    ...
    # OVERALL SCORE
    # -------------------------------
    overall_score = (total_matched / total_required) * 100 if total_required else 0
    ...
    # SHORTLIST DECISION
    # -------------------------------
    result["decision"] = "shortlisted" if overall_score >= 60 else "rejected"
